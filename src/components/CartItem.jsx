...
const CartItem = ({cartItem }) => {
  const {cartID, title, price, image, amount, company, product, productColor} = cartItem
  ...
  return (
    <article key={cartID} className='mb-12 flex flex-col gap-y-4 sm:flex-row flex-wrap border-b border-base-300 pb-6 last:border-b-0'>
      ...
      {/* REMOVE */}
        <button className="mt-2 link lin-primary link-hover text-sm"
        onClick={removeItemFromTheCart}
        >Remove</button>
      </div>
      {/* PRICE */}
      <p className="font-normal sm:ml-auto">{formatPrice(price)}</p>
    </article>
  )
}
